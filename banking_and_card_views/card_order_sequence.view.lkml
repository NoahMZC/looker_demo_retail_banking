...
view: card_order_sequence {
  ...
  derived_table: {
    sql: select card.*, disp.client_id, rank() over (partition by disp.client_id order by card.date) as rank
        from `mzcdsc-team-200716.Looker_Demo_retail_banking.disp` as disp
          inner join `mzcdsc-team-200716.Looker_Demo_retail_banking.card` as card
            on disp.disp_id = card.disp_id;;
    sql_trigger_value: select max(date) from `mzcdsc-team-200716.Looker_Demo_retail_banking.trans`;;
